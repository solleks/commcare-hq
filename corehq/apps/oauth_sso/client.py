...
class MicrosoftClient:
    ...
    def __init__(self, azure_ad_config, state=None, request=None, *args, **kwargs):
        from .conf import config

        self.config = config
        self.azure_ad_config = azure_ad_config
        self.state = state
        self.token = None
    ...
    def authorization_url(self):
        ...
        #                Yes, it's passed to constructor as 'state'.
        ...
        # Prepend the project name to the state value with a $ separator.
        # TODO(Charlie): A better solution would be to store the project
        # name in a distributed cache like Redis, keyed by the state value.
        state = self.azure_ad_config.project + '$' + state
